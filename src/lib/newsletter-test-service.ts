...
export async function processTestNewsletter(userId: string, overrideRecipientEmail?: string) {
  ...
  const userListened = new Set([...user.completedEpisodes, ...user.playbackHistory.filter((h: any) => h.completed).map((h: any) => h.episodeSlug)]);
  ...
  const scoredCandidates = candidates.map(ep => {
    let score = 0;
    for (const tag of (ep.data.tags || [])) score += interestCounts.get(tag) || 0;
    for (const person of (ep.data.participants || [])) score += interestCounts.get(`person:${person}`) || 0;
    return { ep, score };
  });
  ...
  return await sendEmail({
    to: overrideRecipientEmail || user.email,
    toName: user.name,
    subject: `🧪 [TEST] "${recommendations[0].data.title}" - Veredillas FM Premium`,
    htmlContent: html
  });
}
                    ...
                    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                      <tr>
                        ...
                        <td style="padding-left:16px;" valign="top">
                          ...
                          <p style="margin:4px 0 12px; font-size:13px; color:#a1a1aa; line-height:1.4;">${ep.data.description ? ep.data.description.substring(0, 90) : ''}...</p>
...
<head>
  <meta charset="utf-8">
  <title>TEST: ${mainEp.data.title} - Veredillas FM</title>
...
<body style="margin:0; padding:0; background:#000000;">
  <div class="preheader">🧪 [TEST NESTLETTER] "${mainEp.data.title}" en Veredillas FM.</div>
          ...
          <tr style="background: linear-gradient(180deg, #1e1e2d 0%, #0a0a0a 100%);">
            <td align="center" style="padding:40px 32px 24px;">
              ...
              <h1 style="margin:20px 0 0; font-size:32px; line-height:1.2; color:#ffffff; font-weight: 800; letter-spacing: -1px;">
                Hola, <span style="color:#8b5cf6;">${user.name}</span>
              </h1>
                  ...
                  <td style="padding:0;">
                    ...
                    <div style="padding:32px;">
                      ...
                      <p style="margin:0 0 24px; font-size:16px; line-height:1.6; color:#a1a1aa;">
                        ${mainEp.data.description ? mainEp.data.description.substring(0, 220) : ''}...
                      </p>
                    ...
                    <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td style="padding:0 12px;">
                          <a href="https://instagram.com/veredillasfm.es" target="_blank" style="color:#ffffff;">
